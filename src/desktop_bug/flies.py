...
class FlyWorld:
    ...
    def __init__(self, screen_w: float, screen_h: float, *, enabled: bool = True,
                 ...
                 max_flies: int = 6, scale: float = 1.0,
                 rng: Optional[random.Random] = None) -> None:
        # A caller that wants a replayable run passes a seeded `random.Random`
        # so the nest icon's pulse and every fly's own visual randomness
        # retrace identically too (DC-40). `CreatureManager` builds this from
        # its own `seed` but keeps it a separate instance from `_rng`: sharing
        # one object would interleave fly draws into the creature/decision
        # stream, perturbing spider behaviour merely because a fly nest exists
        # on screen. No rng keeps the `random` module itself, matching the
        # rest of the codebase's seeding convention (see `BaseWorld`).
        self.rng = rng or random
        self.screen_w = float(screen_w)
        self.screen_h = float(screen_h)

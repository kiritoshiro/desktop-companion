...
- **Job** is separate from temperament and describes colony work. Choose No job, Hunter, Builder, Guard, Scout, or Web tender. Builders establish and upgrade a visible shared team base; Guards hold a post outside it, facing outwards, and raise an alert when a declared foe enters the perimeter. Guards and Web tenders are also issued the **web trap** ability, so they can pin a foe with silk -- no temperament the dropdown offers grants silk on its own, so without this the trapping that spiders use in a fight was unreachable unless you ticked it by hand in **Abilities**. Jobs add abilities; they do not silently change personality values.

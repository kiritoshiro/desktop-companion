...
def main() -> int:
    ...
    assert antennae["rub_joint_angle"] >= 0.02
    ...
    assert antennae["hand_max_forward"] < 0.60
    assert antennae["hand_max_lateral"] < 0.44
    ...
    assert all(
        abs(target_f - rest_forward) < probe.size * 0.04
        and abs(abs(target_s) - rest_lateral) < probe.size * 0.04
    ...
    assert max(idle_middle_angles) - min(idle_middle_angles) > 0.08
    probe._update_antennae(1.0 / 60.0)
